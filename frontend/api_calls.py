...
# When running locally, the .env file is in the parent directory (root folder)
# When running in Docker, environment variables are already injected via docker-compose.yml env_file
...
def get_document_view_url_api(document_name: str, document_tag: str = "General", version: int = None):
    """
...
def list_minio_documents_api():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Error listing documents: {response.status_code} - {response.text}")
            return None
    except Exception as e:
...
def check_answer_api(question, answer, relevant_documents, question_prompt_key, prompt_template="rag_check", used_model=""):
    ...
    if response.status_code == 200:
        ...
        st.error(f"Check Answer API Error: {response.status_code} - {response.text}")
        return {"evaluation": f"API Error: {response.status_code}"}
    
def check_keywords_api(question, answer, expected_keywords=None, threshold=0.6):
    ...
    if response.status_code == 200:
        ...
        st.error(f"Check Keywords API Error: {response.status_code} - {response.text}")
        return {"evaluation": f"API Error: {response.status_code}"}

def upload_documents_api(files, document_tag="General", chunk_size=2000, chunk_overlap=200, max_workers=5, use_bulk_upload=True):
    """
...
def start_async_upload(files, document_tag="General", chunk_size=2000, chunk_overlap=200, 
                      max_workers=5, use_bulk_upload=True, user_id=None):
    """
...
def get_full_document(document_source: str) -> str:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Document API Error: {response.status_code} - {response.text}")
            return None
    except requests.exceptions.RequestException as e:
        st.error(f"Connection error to loader service: {str(e)}")
        return None

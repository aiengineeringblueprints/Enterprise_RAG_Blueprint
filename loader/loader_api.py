...
class UploadJob(BaseModel):
    ...
    phases: Dict[str, JobPhase] = {
        "minio_upload": JobPhase(
            name="MinIO Upload", status=JobStatus.PENDING
        ),
        "chunking": JobPhase(name="Text Chunking", status=JobStatus.PENDING),
        "embedding": JobPhase(
            name="Embedding Generation", status=JobStatus.PENDING
        ),
        "vector_db": JobPhase(
            name="Vector DB Storage", status=JobStatus.PENDING
        ),
    }
...
async def get_document_categories():
    ...
    return {"categories": list(TAG_BITMASK_MAPPING.keys())}
...
async def upload_documents_async(
    ...
    session_id = (
        f"async_upload_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{job_id[:8]}"
    )
    ...
    background_tasks.add_task(
        process_documents_background, job_id, str(temp_path)
    )
...
async def list_upload_jobs(
    limit: int = 50, status_filter: Optional[JobStatus] = None
):
    ...
    with jobs_lock:
        ...
        return JobListResponse(
            jobs=job_responses, total_jobs=len(jobs_storage)
        )
...
async def get_full_document(document_source: str):
    ...
    try:
        ...
        if matching_source:
            ...
            if content and content.strip():  # Check for non-empty content
                ...
                return {
                    ...
                    "original_request": document_source,
                    "content": content
                    or "Dokument gefunden, aber Inhalt ist leer oder sehr kurz.",
                    "success": True,
        ...
        similar_files = [
            ...
            if requested_filename.lower() in src.lower()
        ]
...
async def list_minio_objects():
    ...
    try:
        ...
        objects = []
        for obj in versioning.client.list_objects(
            versioning.bucket_name, recursive=True
        ):
            objects.append(
                {
...
async def get_document_versions(
    document_name: str, document_tag: str = "General"
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving versions: {str(e)}"
        )


@app.get("/document_version/{document_name}/{version}")
async def get_specific_document_version(
    document_name: str, version: int, document_tag: str = "General"
):
    """
    ...
    try:
        ...
        content = versioning.get_document_version(
            document_name, document_tag, version
        )
...
async def generate_view_url(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error generating view URL: {str(e)}"
        )


@app.get("/document/view/{document_name}")
async def view_document(
    document_name: str, version: int = None, document_tag: str = "General"
):
    """
    ...
    try:
        logger.info(
            f"View document request: name={document_name}, tag={document_tag}, version={version}"
        )
        versioning = MinIODocumentVersioning()

        # Try to get metadata (but allow fallback if not available)
        logger.info(
            f"Loading metadata for document: {document_name}, tag: {document_tag}"
        )
        metadata = versioning._load_document_metadata(
            document_name, document_tag
        )
        ...
        if not has_metadata:
            logger.warning(
                f"No metadata found for document: {document_name}, tag: {document_tag}"
            )
            logger.info("Will attempt direct access as fallback...")
        else:
            logger.info(
                f"Metadata found: {len(metadata.get('versions', []))} version(s)"
            )
        ...
        if content is None:
            logger.error(
                f"Document content not found: {document_name}, tag: {document_tag}, version: {version}"
            )
            raise HTTPException(
                status_code=404, detail=f"Document not found: {document_name}"
            )
        ...
        if has_metadata:
            ...
            logger.info(
                "Using fallback: inferring file extension from document name"
            )
        ...
        if file_extension.lower() != ".pdf":
            safe_filename = f"{document_name}{file_extension}".replace(
                " ", "_"
            )
...
async def debug_list_documents(document_tag: str = None):
    ...
    try:
        ...
        objects = versioning.client.list_objects(
            versioning.bucket_name, recursive=True
        )
        ...
        for meta_key in metadata_objects:
            try:
                response = versioning.client.get_object(
                    versioning.bucket_name, meta_key
                )
                ...
                if (
                    document_tag is None
                    or metadata.get("document_tag") == document_tag
                ):

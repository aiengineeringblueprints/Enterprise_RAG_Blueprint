...
def create_test_documents_in_vectorstore():
    """
    Create test documents directly in the vectorstore for testing purposes.
    This bypasses the normal document loading process.
    """
    try:
        from langchain_core.documents import Document

        # Create test documents with known sources
        test_docs = [
        Document(
                page_content="Dies ist der erste Teil des Test-Dokuments über PDF-Dateien. Es enthält wichtige Informationen über die Funktionsweise von RAG-Systemen und wie sie Dokumente verarbeiten.",
                metadata={
            "source": "test_documents/pdf_test_1_BA.pdf",
                    "page": 1,
                    "chunk_index": 0,
                    "category_bitmask": 1  # General
                }
            ),
            Document(
                page_content="Der zweite Teil des PDF-Dokuments beschreibt die technischen Details der Implementierung. Hier werden verschiedene Algorithmen und Ansätze für die Dokumentenverarbeitung erklärt.",
                metadata={
                    "source": "test_documents/pdf_test_1_BA.pdf", 
                    "page": 1,
                    "chunk_index": 1,
            ...
            Document(
                page_content="Dies ist ein Text-Dokument über römische Geschichte. Es behandelt die Entstehung des römischen Reiches und dessen Einfluss auf die moderne Welt.",
                metadata={
                    "source": "test_documents/txt_test_1_rom.txt",
                    "page": 0,
                    "chunk_index": 0,
                    "category_bitmask": 1
                }
            ...
            Document(
                page_content="Fortsetzung der römischen Geschichte mit Details über die Eroberungen und die politische Struktur des Reiches.",
                metadata={
                    "source": "test_documents/txt_test_1_rom.txt",
                    "page": 0, 
                    "chunk_index": 1,
        ...
        # Upload to vectorstore (local)
        upload_documents_to_vectorstore(test_docs)
        return f"Created {len(test_docs)} test documents"
    except Exception as e:

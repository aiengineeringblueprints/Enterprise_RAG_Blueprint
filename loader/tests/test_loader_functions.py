...
class TestMultiEncodingLoader:
    ...
    def test_load_utf8_file(self, temp_test_dir):
        ...
        file_path.write_text("UTF-8 content with special chars: äöü", encoding="utf-8")
...
class TestLoadDocsFromDict:
    ...
    def test_load_with_tag(self, temp_test_dir):
        ...
        docs = load_docs_from_dict(str(temp_test_dir), document_tag="IT & Technology")
    ...
    def test_load_with_error_handling(self, mock_pdf_loader, temp_test_dir):
        ...
        mock_pdf_loader.return_value.load.side_effect = Exception("Loading failed")
...
class TestUploadAllDocsToVectorStore:
    ...
    def test_upload_with_default_params(
        ...
        mock_splitter_class.assert_called_once_with(chunk_size=2000, chunk_overlap=200)
    ...
    def test_upload_with_custom_chunk_params(
        ...
        mock_splitter_class.assert_called_once_with(chunk_size=1000, chunk_overlap=100)
        
    @patch('loader_functions.upload_documents_to_vectorstore')
    @patch('loader_functions.load_docs_from_dict')
    def test_upload_with_tag(self, mock_load_docs, mock_upload, mock_documents):

        mock_load_docs.return_value = mock_documents
        
        upload_all_docs_to_vector_store(
            root_path="test_path",
            document_tag="Finance & Controlling"
        )
    ...
    def test_upload_text_splitting(
        ...
        mock_splitter.split_documents.return_value = [
            Document(page_content="x" * 2000, metadata={"source": "large.txt"}),
            Document(page_content="x" * 2000, metadata={"source": "large.txt"}),
            Document(page_content="x" * 1000, metadata={"source": "large.txt"})
        ]
        mock_splitter_class.return_value = mock_splitter
